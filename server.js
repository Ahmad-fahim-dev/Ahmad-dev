...
app.put('/api/blogs/:id', authenticateToken, upload.single('image'), (req, res) => {
  ...
  if (req.file) {
    ...
    if (blogs[index].image) {
      const oldImagePath = path.join(__dirname, blogs[index].image);
      if (fs.existsSync(oldImagePath)) fs.unlinkSync(oldImagePath);
    }
    blogs[index].image = `/uploads/${req.file.filename}`;
  }
...
app.put('/api/projects/:id', authenticateToken, upload.single('image'), (req, res) => {
  ...
  if (req.file) {
    if (projects[index].image) {
      const oldImagePath = path.join(__dirname, projects[index].image);
      if (fs.existsSync(oldImagePath)) fs.unlinkSync(oldImagePath);
    }
